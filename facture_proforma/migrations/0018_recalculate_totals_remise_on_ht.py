...
def recalculate_totals(apps, schema_editor):
    ...
    FactureProFormaLine = apps.get_model("facture_proforma", "FactureProFormaLine")

    doc_ids = list(
        FactureProFormaLine.objects.values_list(
            "facture_pro_forma_id", flat=True
        ).distinct()
    )
    for i in range(0, len(doc_ids), BATCH_SIZE):
        ...
        for doc in docs:
            _recalc(doc, FactureProFormaLine, "facture_pro_forma")
        FactureProForma.objects.bulk_update(
            docs,
...
class Migration(migrations.Migration):
    dependencies = [
        ("facture_proforma", "0017_quantity_to_decimal"),
    ]
