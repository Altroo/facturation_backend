...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            if created:
                self.stdout.write(self.style.SUCCESS("  ✓ Created group: Caissier (full access)"))
            else:
                self.stdout.write(self.style.WARNING("  - Group already exists: Caissier"))

            comptable, created = Group.objects.get_or_create(name="Comptable")
            if created:
                self.stdout.write(self.style.SUCCESS("  ✓ Created group: Comptable (read & print only)"))
            else:
                self.stdout.write(self.style.WARNING("  - Group already exists: Comptable"))

            commercial, created = Group.objects.get_or_create(name="Commercial")
            if created:
                self.stdout.write(self.style.SUCCESS("  ✓ Created group: Commercial (create documents)"))
            else:
                self.stdout.write(self.style.WARNING("  - Group already exists: Commercial"))

            lecture, created = Group.objects.get_or_create(name="Lecture")
            if created:
                self.stdout.write(self.style.SUCCESS("  ✓ Created group: Lecture (view only)"))
            else:
                self.stdout.write(self.style.WARNING("  - Group already exists: Lecture"))
