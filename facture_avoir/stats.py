...
def get_avoir_stats_by_currency(company_id: int) -> dict:
    ...
    for devise in devises:
        total = FactureAvoir.objects.filter(
            company_id=company_id,
            devise=devise,
            statut__in=AVOIR_ACTIVE_STATUSES,
        ).aggregate(total=Sum("total_ttc_apres_remise"))["total"] or Decimal("0.00")
        total_tva = FactureAvoir.objects.filter(
            company_id=company_id,
            devise=devise,
            statut__in=AVOIR_ACTIVE_STATUSES,
        ).aggregate(total=Sum("total_tva"))["total"] or Decimal("0.00")
        stats_by_currency[devise] = {
            "total_avoirs": format(total, "f"),
            "total_tva": format(total_tva, "f"),
        }
