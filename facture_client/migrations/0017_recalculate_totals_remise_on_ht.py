...
def recalculate_totals(apps, schema_editor):
    ...
    FactureClientLine = apps.get_model("facture_client", "FactureClientLine")

    doc_ids = list(
        FactureClientLine.objects.values_list("facture_client_id", flat=True).distinct()
    )
    for i in range(0, len(doc_ids), BATCH_SIZE):
        batch_ids = doc_ids[i : i + BATCH_SIZE]
        docs = list(FactureClient.objects.filter(pk__in=batch_ids))
        for doc in docs:
            _recalc(doc, FactureClientLine, "facture_client")
        FactureClient.objects.bulk_update(
            docs,
...
class Migration(migrations.Migration):
    dependencies = [
        ("facture_client", "0016_quantity_to_decimal"),
    ]
