...
class FactureClientListCreateView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        base_queryset = FactureClient.objects.filter(client__company_id=company_id)
        filterset = FactureClientFilter(request.GET, queryset=base_queryset)
        ordered_qs = filterset.qs.order_by("-id")
        if pagination:
            paginator = CustomPagination()
            page = paginator.paginate_queryset(ordered_qs, request)
            serializer = FactureClientListSerializer(
    ...
    def post(request, *args, **kwargs):
        ...
        if not client:
            raise PermissionDenied(
                _("Un client doit être spécifié pour la facture client.")
            )
        try:
        ...
        if not Membership.objects.filter(
            ...
            raise PermissionDenied(
                _(
                    "Vous n'êtes pas autorisé à créer une facture client pour cette société."
                )
            )
        ...
        response_serializer = FactureClientDetailSerializer(
            instance, context={"request": request}
        )
        return Response(response_serializer.data, status=status.HTTP_201_CREATED)


class FactureClientDetailEditDeleteView(APIView):
    permission_classes = (permissions.IsAuthenticated,)
    ...
    def get_object(pk):
        try:
            return FactureClient.objects.get(pk=pk)
        except FactureClient.DoesNotExist:
            raise Http404(_("Aucun facture client ne correspond à la requête."))

    def get(self, request, pk, *args, **kwargs):
        object_ = self.get_object(pk)
        if not self._has_membership(request.user, object_.client.company_id):
            raise PermissionDenied(
                _("Vous n'êtes pas autorisé à consulter cette facture client.")
            )
    ...
    def delete(self, request, pk, *args, **kwargs):
        ...
        if not self._has_membership(request.user, object_.client.company_id):
            raise PermissionDenied(
                _("Vous n'êtes pas autorisé à supprimer cette facture client.")
            )
        object_.delete()
...
class GenerateNumeroFactureView(APIView):
    ...
    def get(request, *args, **kwargs):
        new_num = get_next_numero_facture_client()
        return Response({"numero_facture": new_num}, status=status.HTTP_200_OK)


class FactureClientStatusUpdateView(APIView):
    permission_classes = (permissions.IsAuthenticated,)
    ...
    def patch(self, request, pk, *args, **kwargs):
        ...
        valid_statuses = [choice[0] for choice in FactureClient.STATUT_CHOICES]
