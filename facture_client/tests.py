...
class TestFactureClientAPI(SharedDocumentAPITestsMixin):
    ...
    def test_get_facture_client_detail_not_found_message_is_user_friendly(self):
        response = self.client_api.get(self._detail_url(99999))
...
# -----------------------------------------------------------------------------
@pytest.mark.django_db
class TestBulkDeleteFactureClientAPI:
    def setup_method(self):
        self.user = CustomUser.objects.create_user(
            email="bulk_fc@example.com", password="pass"
        )
        ...
        self.client_obj = Client.objects.create(
            code_client="BDFC001",
            client_type="PM",
            raison_sociale="BulkFC Client",
            ville=self.ville,
            company=self.company,
        )
        self.fc1 = FactureClient.objects.create(
            numero_facture="BKFC/01",
            client=self.client_obj,
            date_facture="2025-01-01",
            mode_paiement=self.mode_paiement,
            statut="Brouillon",
        ...
        self.fc2 = FactureClient.objects.create(
            numero_facture="BKFC/02",
            client=self.client_obj,
            date_facture="2025-01-02",
            mode_paiement=self.mode_paiement,
            statut="Brouillon",
    ...
    def test_bulk_delete_unauthenticated_returns_401(self):
        url = reverse("facture_client:facture-client-bulk-delete")
        anon = APIClient()
        response = anon.delete(url, {"ids": [self.fc1.id]}, format="json")
    ...
    def test_bulk_delete_wrong_company_returns_403(self):
        ...
        other_client = Client.objects.create(
            code_client="OTHFC01",
            client_type="PM",
            raison_sociale="Other FC Client",
            ville=other_ville,
            company=other_company,
        )
        other_fc = FactureClient.objects.create(
            numero_facture="OTHER/01",
            client=other_client,
            date_facture="2025-01-01",
