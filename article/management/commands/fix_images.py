...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(image_folder_path):
            ...
            self.stdout.write(
                self.style.SUCCESS(f'  ✓ Folder created. Please upload images to this folder.')
            )
            return
        ...
        for image_file in sorted(image_files):
            ...
            if reference is None:
                self.stdout.write(
                    self.style.WARNING(f'  [X] {base_name}: Article not found in company {company_id} (tried: {", ".join(possible_references)})')
                )
                not_found_count += 1
                continue
            ...
            if dry_run:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f'  [OK] {reference}: {action} to {photo_path}'
                    )
                )
            else:
        ...
        if skipped_has_photo > 0:
            self.stdout.write(
                self.style.WARNING(f'  - Skipped (already had photo): {skipped_has_photo}')
            )
