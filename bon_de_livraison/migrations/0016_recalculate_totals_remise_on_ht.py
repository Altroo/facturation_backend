...
def recalculate_totals(apps, schema_editor):
    BonDeLivraison = apps.get_model("bon_de_livraison", "BonDeLivraison")
    BonDeLivraisonLine = apps.get_model("bon_de_livraison", "BonDeLivraisonLine")

    doc_ids = list(
        BonDeLivraisonLine.objects.values_list(
            "bon_de_livraison_id", flat=True
        ).distinct()
    )
    for i in range(0, len(doc_ids), BATCH_SIZE):
        batch_ids = doc_ids[i : i + BATCH_SIZE]
        docs = list(BonDeLivraison.objects.filter(pk__in=batch_ids))
        for doc in docs:
            _recalc(doc, BonDeLivraisonLine, "bon_de_livraison")
        BonDeLivraison.objects.bulk_update(
            docs,
...
class Migration(migrations.Migration):
    dependencies = [
        ("bon_de_livraison", "0015_quantity_to_decimal"),
    ]
